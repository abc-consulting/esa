export const dom = {
  searchBtn: document.getElementById('searchBtn'),
  areaBtn: document.getElementById('areaBtn'),
  siteSelect: document.getElementById('siteSelect'),
  searchInput: document.getElementById('searchInput'),
  areaInput: document.getElementById('areaInput'),
  filterBar: document.getElementById('filter-bar'),
  filterInput: document.getElementById('filterInput'),
  filterChips: document.getElementById('filter-chips'),
  status: document.getElementById('status'),
  selectedOutput: document.getElementById('selected-output'),
  contentLayout: document.getElementById('content-layout'),
  profilesContainer: document.getElementById('profiles-container'),
  profileDetailsContainer: document.getElementById('profile-details-container'),
  imagesContainer: document.getElementById('images-container'),
  favoritesContainer: document.getElementById('favorites-container'),
  areaDropdownWrap: document.getElementById('area-dropdown-wrap'),
  areaDropdownBtn: document.getElementById('areaDropdownBtn'),
  areaDropdownPanel: document.getElementById('areaDropdownPanel'),
  tagDropdownWrap: document.getElementById('tag-dropdown-wrap'),
  tagDropdownBtn: document.getElementById('tagDropdownBtn'),
  tagDropdownPanel: document.getElementById('tagDropdownPanel'),
  redvelvetDetailFilters: document.getElementById('redvelvet-detail-filters'),
  ageMinInput: document.getElementById('ageMinInput'),
  ageMaxInput: document.getElementById('ageMaxInput'),
  bandSelect:    document.getElementById('bandSelect'),
  cupSelect:     document.getElementById('cupSelect'),
  sizeMinBtn:   document.getElementById('sizeMinBtn'),
  sizeMinPanel: document.getElementById('sizeMinPanel'),
  sizeMaxBtn:    document.getElementById('sizeMaxBtn'),
  sizeMaxPanel:  document.getElementById('sizeMaxPanel'),
  scrapeModeBtn:    document.getElementById('scrapeModeBtn'),
  adminBtn:         document.getElementById('adminBtn'),
  adminModal:       document.getElementById('admin-modal'),
  adminCloseBtn:    document.getElementById('adminCloseBtn'),
  adminFailedCount: document.getElementById('adminFailedCount'),
  adminRetryBtn:    document.getElementById('adminRetryBtn'),
  adminRetryStatus: document.getElementById('adminRetryStatus'),
};
